...
const VendorCreditInvoicesTab = () => {
    ...
    const fetchMyInvoices = async () => {
        try {
            ...
            const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/finance/invoice/received`, {
    ...
    const getLSID = (id) => {
        ...
        return 'LS-' + (1000000000 + Math.abs(hash)).toString();
    };

    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            {error && <div className="p-4 bg-red-50 text-red-600 rounded-xl text-sm font-bold border border-red-100 flex items-center gap-2"><XCircle size={16} />{error}</div>}
            {success && <div className="p-4 bg-green-50 text-green-600 rounded-xl text-sm font-bold border border-green-100 flex items-center gap-2"><CheckCircle2 size={16} />{success}</div>}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-6 border-b border-slate-100 flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-[#0B1E43]">Credit Invoices (Against Me)</h2>
                        <p className="text-sm font-semibold text-slate-500">Track invoices submitted against your LS ID</p>
                    </div>
                </div>
                                    ...
                                    <tr key={inv._id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-sm font-black text-[#0B1E43] uppercase">{inv.vendor?.company || inv.vendor?.name || 'N/A'}</span>
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Vendor ID: {getLSID(inv.vendor?._id)}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
...
export default VendorCreditInvoicesTab;
