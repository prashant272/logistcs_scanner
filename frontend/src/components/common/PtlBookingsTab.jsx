                            ...
                            {bookings.length === 0 ? (
                                ...
                                <tr key={booking._id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="text-[10px] text-slate-500 mt-1">Base Freight: ₹{booking.charged_price || 0}</div>
                                        {(booking.vendor_markup_fee > 0) && (
                                            <div className="text-[10px] text-green-600 font-medium">Your Markup: ₹{booking.vendor_markup_fee}</div>
                                        )}
                                        ...
                                        {booking.delhivery_lr_number ? (
                                                    ...
                                                    onClick={async () => {
                                                        try {
                                                            ...
                                                            if (res.data?.images && res.data.images.length > 0) {
                                                                // Delhivery returned base64 images
                                            ...
                                            <button
                                                onClick={async () => {
                                                    try {
                                                        const token = localStorage.getItem('userToken');
                                                        const btn = document.getElementById(`refresh-btn-${booking._id}`);
                                                        if (btn) btn.innerHTML = 'Checking...';
                                                        await axios.get(`${import.meta.env.VITE_API_BASE_URL}/delhivery/manifest/status/${booking._id}`, {
                                                            headers: { Authorization: `Bearer ${token}` }
                                                        });
                                                        fetchBookings(); // refresh table
                                                    } catch (e) {
                                                        alert(e.response?.data?.message || "Still processing or failed");
                                                        fetchBookings();
                                                    }
                                                }}
                                                id={`refresh-btn-${booking._id}`}
                                                className="bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors whitespace-nowrap"
                                            >
                                                Refresh Status
                                            </button>
                                        ) : null}
