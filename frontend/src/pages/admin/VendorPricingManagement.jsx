...
import { Plus, Trash2, ToggleLeft, ToggleRight, X, Calendar, DollarSign, Clock, Tag, Edit, Building, Search, RefreshCw } from 'lucide-react';
...
const VendorPricingManagement = () => {
  ...
  const fetchRates = async (vendorId) => {
    try {
      setLoading(true);
      setError('');
      const token = localStorage.getItem('adminToken');
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/admin/pricing/${vendorId}`, config);
      setRates(data || []);
      setLoading(false);
    } catch (err) {
      console.error('Error fetching rates:', err);
      setError('Failed to load rates for this vendor');
      setLoading(false);
    }
  };
  ...
  const handleToggleStatus = async (id) => {
    try {
      ...
      const { data } = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/admin/pricing/${id}/toggle`, {}, config);
      
      setRates(prev => prev.map(r => r._id === id ? { ...r, status: data.status } : r));
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/admin/pricing/${id}`, config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRateId) {
        const { data } = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/admin/pricing/${editingRateId}`, payload, config);
        setRates(prev => prev.map(r => r._id === editingRateId ? data : r));
      } else {
        const { data } = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/admin/pricing`, payload, config);
      ...
      setFormError(err.response?.data?.message || 'Error saving rate');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#0B1E43] tracking-tight">Manage Vendor Pricing</h1>
          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mt-0.5">Select a transport partner to inspect and manage their active base rates</p>
        </div>

        {/* Vendor Selection Dropdown */}
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-2 bg-white border border-slate-200/80 rounded-xl px-3 py-2.5 shadow-sm">
            <Building size={16} className="text-[#0066FF]" />
            <select
              value={selectedVendorId}
              onChange={(e) => setSelectedVendorId(e.target.value)}
              className="bg-transparent text-xs font-black text-slate-800 outline-none pr-6 cursor-pointer"
          ...
          <button 
            onClick={() => selectedVendorId && fetchRates(selectedVendorId)} 
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_12px_40px_rgba(11,30,67,0.025)] space-y-6">
        <div className="flex justify-between items-center border-b border-slate-100 pb-4 flex-wrap gap-4">
          <div>
            <h3 className="text-xs font-black text-[#0B1E43] uppercase tracking-widest flex items-center gap-1.5">
              <DollarSign size={15} className="text-[#0066FF]" /> Active Vendor Rates
            </h3>
          </div>
          <button
            onClick={() => { resetForm(); setIsModalOpen(true); }}
            className="bg-[#0066FF] hover:bg-[#0052cc] text-white text-xs font-bold px-4 py-2.5 rounded-xl flex items-center gap-1.5 transition-all shadow-sm shadow-[#0066FF]/10 cursor-pointer uppercase tracking-wider"
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400 font-bold text-xs uppercase tracking-wider">
            No pricing records configured for this vendor.
          </div>
        ) : (
                ...
                {rates.map((r) => (
                  <tr key={r._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      >
                        {r.status === 'active' ? (
                          <div className="flex items-center gap-1 text-green-600 bg-green-50 px-2.5 py-1 rounded-lg border border-green-100">
                            <ToggleRight size={20} className="text-green-500" />
                            <span className="text-[9px] font-black uppercase">Active</span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1 text-slate-400 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-200">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-[0_24px_60px_rgba(11,30,67,0.15)] border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50/50">
              <div>
                <h3 className="text-base font-black text-[#0B1E43] tracking-tight">{editingRateId ? 'Edit Vendor Rate' : 'Add Vendor Pricing Rate'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 flex-1">
              {formError && (
                <div className="bg-red-50 border border-red-100 text-red-650 rounded-xl p-3.5 text-xs font-bold">
                  {formError}
                </div>
              )}
